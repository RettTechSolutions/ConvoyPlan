...
- **In der Fahrzeugliste der Tracking-Ansicht lagen die Angaben am Telefon übereinander.** Das „LIVE"-Abzeichen stand mitten in der Mannschaftsstärke, der Fahrzeugname war auf einen Strich zusammengeschoben — betroffen waren die Tracking-Ansicht und der Fahrer-Link gleichermaßen.
  ...
  Dazu ist die Schublade am Telefon breiter geworden — `min(400px, 90vw)` statt 320 px, 300 px und 88 vw: ein Wert für Tracking-Ansicht, Fahrer-Link **und** Planung, wo vorher drei standen. Die Leiste ist dort die Arbeitsfläche; die Karte dahinter braucht nur so viel Rand, dass ein Tippen darauf die Schublade wieder schließt.
...
- **Ein Umsortieren der Wegpunkte änderte die Route nicht und war nach der Berechnung wieder weg.** Die Reihenfolge ließ sich ziehen und wurde auch gespeichert — die Routenberechnung ignorierte sie und schrieb sie anschließend zurück auf den alten Stand.
  ...
  Die Projektion auf die vorherige Route bleibt dort, wofür sie gedacht war: Die automatisch vorgeschlagenen **Technischen Halte und Tankstopps** hängt die Oberfläche ans Listenende, obwohl sie in der Mitte liegen. Sie werden einmal eingeordnet — danach stehen sie in der Liste und lassen sich verschieben wie jeder andere Wegpunkt. Der Umweg von mehreren hundert Kilometern, gegen den die Projektion eingeführt wurde, kommt damit nicht zurück.

  Welcher Wegpunkt eingeordnet wird, entscheidet dabei seine **Herkunft** und nicht seine Position: Ein Vorschlag der Anwendung wird beim Anlegen als „noch nicht einsortiert" vermerkt, und der Vermerk verfällt, sobald er einen Platz hat — durch die Berechnung, durch Ziehen in der Liste oder durch ein ausdrücklich gesetztes Ziel. Ein Technischer Halt, den jemand bewusst als **letzten** Wegpunkt setzt, bleibt damit auch dort. Bestehende Konvois behalten bei der ersten Berechnung nach dem Update genau das bisherige Verhalten (Migration `0046`).
...
- **Ein KI-Programm kann jetzt an einem Konvoi „dranbleiben", statt ihn abzufragen.** Wer eine Marschkolonne begleitet, wollte bislang im Sekundentakt nach den Positionen fragen — das kostet Kontingent, Kontext und Nerven. Neu lässt sich ein Konvoi **abonnieren**: der Server meldet sich, sobald sich dort etwas bewegt. Übertragen wird dabei nur der Anstoß „hier gibt es Neues"; die Daten holt das Programm anschließend selbst, und dabei greift dieselbe Prüfung wie bei jedem anderen Zugriff. Voraussetzung ist ein Programm, das die MCP-Revision 2026-07-28 spricht.

  Die Adresse eines Abonnements trägt die **Organisation in sich**. Das ist kein Schönheitsfehler, sondern die Trennung selbst: das verwendete MCP-SDK nimmt jede angefragte Adresse ohne Rückfrage an, ein Programm könnte also die Adresse eines fremden Konvois einfach behaupten. Es bekäme dessen Daten zwar nie zu sehen — aber die Nachricht, *dass* sich dort gerade etwas tut, und im Einsatz ist schon das eine Auskunft. Weil die Organisation in der Adresse steht, passt eine fremde Adresse schlicht nicht, und zwar ohne Nachschlagen und ohne Zeitfenster, in dem die Prüfung danebenliegen könnte. Zwei Tests halten das fest: einer fährt den echten Abo-Weg ab, ein zweiter nagelt das ungeprüfte Annehmen des SDK fest, damit es auffällt, wenn es sich ändert.

- **Reicht das Recht eines Zugangs nicht, sagt der Server jetzt, welches fehlt.** Bisher scheiterte ein Schreibversuch mit einem Lese-Zugang an einer Fehlermeldung, die das Programm nicht verwerten konnte — der Benutzer musste die Verbindung von Hand neu einrichten. Jetzt antwortet der Server protokollgerecht mit der Angabe, welches Recht gebraucht wird; ein Programm fordert es daraufhin von sich aus nach, und der Benutzer bestätigt einmal im Browser. Was dabei erteilt werden kann, deckelt weiterhin die Rolle: ein *beobachter* bekommt Schreibrechte auch dann nicht, wenn das Programm danach fragt.

- **Programme können sich über eine hinterlegte Steckbrief-Adresse ausweisen, statt sich selbst einzutragen** (`MCP_ALLOW_CIMD`, standardmäßig aus). Bei der Selbstregistrierung kann sich jedes Programm eintragen, das die Instanz erreicht, und sich dabei nennen wie es will — deshalb steht der Name auf dem Zustimmungsbildschirm als *ungeprüft*. Beim neuen Verfahren nennt ein Programm als Kennung eine HTTPS-Adresse, unter der sein Steckbrief liegt; die Kennung ist damit selbst überprüfbar.

  Eingeschaltet ruft der Server damit eine Adresse ab, **die der Client bestimmt** — die klassische Zutat für einen Angriff auf das interne Netz. Der Abrufer lässt deshalb nur HTTPS zu, folgt keiner Weiterleitung, bricht bei Zeit- und Größenüberschreitung ab und prüft **jede** aufgelöste Adresse, nicht nur die erste. Dabei ist eine Lücke im bisherigen Muster des Projekts aufgefallen: der Adressbereich `100.64.0.0/10` (Carrier-Grade NAT) gilt in Python nicht als „privat" und wäre durchgerutscht; ein Test hält diese Annahme jetzt fest. Standardmäßig bleibt das Verfahren trotzdem aus — das ist eine Entscheidung des Betreibers.

- **Die KI-Schnittstelle lässt sich jetzt im Admin-Portal verwalten — und erteilte Zugänge dort auch wieder entziehen.** Bislang gab es den MCP-Server, aber keine Stelle, an der man sah, wer eigentlich verbunden ist. Ein Widerruf ging nur über den OAuth-Endpunkt `/revoke`, also nur mit dem Client in der Hand — genau dann nicht, wenn man ihn braucht. Der neue Reiter **MCP** zeigt den Zustand der Schnittstelle, die Adresse zum Kopieren, die registrierten Programme und die aktiven Verbindungen. Zwei Knöpfe: **Verbindung trennen** entzieht einem Benutzer den Zugang für ein bestimmtes Programm, **Programm sperren** trennt alle seine Verbindungen und verhindert eine neue Autorisierung. Beides zusammen ist wichtig: ein gesperrtes Programm, dessen Tokens weiterleben, behielte seinen Zugang bis zu deren Ablauf — die Sperre wäre eine Attrappe. Ein Test hält fest, dass ein getrenntes Token sich tatsächlich nicht mehr einlösen lässt.

  In beiden Listen ist der **Name des Programms als ungeprüft gekennzeichnet** und die überprüfte Zieladresse daneben als geprüft — dieselbe Unterscheidung wie auf dem Zustimmungsbildschirm. Bei Selbstregistrierung kann sich jedes Programm eintragen, das die Instanz erreicht, und sich dabei nennen wie es will.

  Der Reiter nennt außerdem das **Zeitfenster beim Widerruf**: die Verbindung ist sofort weg, ein bereits ausgestelltes Zugriffstoken bleibt aber bis zu 15 Minuten gültig (`MCP_ACCESS_TOKEN_TTL_MINUTES`). Das ist der Preis zustandsloser Tokens, und der Bestätigungsdialog sagt es vor dem Klick statt danach.

  Neu ist auch die Anwenderdokumentation: **[MCP-Server](https://github.com/RettTechSolutions/ConvoyPlan/wiki/MCP-Server)** im Wiki beschreibt, was die Schnittstelle kann und was ausdrücklich nicht, wie man ein Programm Schritt für Schritt verbindet, welche Rolle welche Rechte erteilt, wie man einen Zugang wieder entzieht — und ordnet den Datenschutz ein: was ein Modell liest, verlässt die Instanz und geht an den Anbieter des KI-Programms. Für ein BOS-Produkt ist das der Punkt, an dem Kunden zuerst nachfragen, und er steht deshalb ausformuliert da statt zwischen den Zeilen.

- **Der MCP-Server kann jetzt auch schreiben — Konvois planen, Fahrzeuge zuordnen, Wegpunkte setzen, Routen berechnen, Marschstatus melden.** Zwölf neue Werkzeuge ergänzen die neun lesenden aus der ersten Ausbaustufe: Konvoi und Fahrzeug anlegen und ändern, Fahrzeuge zuordnen und die Marschfolge setzen, Wegpunkte anlegen, ändern und umsortieren, Route berechnen, Fahrzeugstatus melden. Dazu drei Resources (Marschbefehl als PDF, Route als GPX, Konvoi als JSON) und eine Prompt-Vorlage, die durch das Ausfüllen eines Marschbefehls führt.

  **Gelöscht wird weiterhin nichts.** Die einzige Ausnahme von der Anlegen-und-Ändern-Regel ist `fahrzeug_aus_konvoi_entfernen`, und die löst nur die Zuordnung: Fahrzeug und Konvoi bleiben bestehen, die Antwort sagt ausdrücklich welches Fahrzeug aus welchem Konvoi gelöst wurde und womit sich das rückgängig machen lässt. Die Positivliste der Werkzeugnamen deckt jetzt beide Ausbaustufen ab.

  **Die Schreiblogik existiert genau einmal.** Die Werkzeuge rufen dieselben Funktionen auf, die hinter der REST-API stehen — direkt, nicht über HTTP. Nachgebaut liefen die beiden Kopien beim ersten Fehler auseinander, den jemand nur an einer Stelle behebt, und in den Routen steckt Logik, die man beim Nachbauen nicht errät: die lückenlose Marschposition beim Zuordnen, das Umsortieren der Wegpunkte entlang der vorher berechneten Route. Die Werkzeugschicht steuert bei, was die Route nicht wissen kann.

  **Jeder Schreibaufruf hinterlässt eine Spur.** Ein Audit-Eintrag mit Benutzer, Organisation, Werkzeugname, Parametern und der `client_id` des MCP-Clients, gekennzeichnet als Quelle `mcp`. Wer später nachvollziehen muss, wie eine Marschkolonne zu ihrem Auftrag kam, sieht damit, ob ein Mensch im Portal oder ein Modell über die Schnittstelle gehandelt hat. Lesende Aufrufe schreiben bewusst nichts — sonst wäre das Log nach einer Woche Modellbetrieb unlesbar.

  **Ohne gültige Lizenz erscheinen die schreibenden Werkzeuge gar nicht.** Der MCP-Server verhält sich damit wie die REST-API im Demo-Modus (lesen ja, schreiben nein), zeigt die Werkzeuge aber erst gar nicht an, statt sie anzubieten und beim Aufruf abzulehnen — ein Modell, das ein Werkzeug sieht, probiert es aus. Geprüft wird bei jeder Auflistung, nicht einmalig beim Start: eine im Admin-Portal hinterlegte Lizenz wirkt ohne Neustart.

  **Kontingente greifen auch hier.** `route_berechnen` kostet Rechenzeit auf dem Routing-Dienst und zählt gegen dieselbe `QUOTA_ROUTING_PER_HOUR`-Einstellung wie die REST-API — zwei Zahlenwelten für dasselbe Kontingent wären eine Einladung zum Auseinanderlaufen. Zusätzlich begrenzt `MCP_TOOL_CALLS_PER_MINUTE` (Standard 120) die Aufrufe je Verbindung: ein Modell in einer Schleife ist ein realistisches Lastprofil, und anders als ein Mensch merkt es nicht von selbst, dass es sich im Kreis dreht. Beide Meldungen sagen dem Modell im Klartext, was zu tun ist.

- **ConvoyPlan kann jetzt als MCP-Server angebunden werden — KI-Clients lesen Konvois, Fahrzeuge, Wegpunkte, Routen und Status direkt aus der eigenen Instanz.** Unter `https://<domain>/mcp` läuft ein Model-Context-Protocol-Server, den Claude Desktop, Claude Code oder claude.ai als Remote-Server eintragen können. Er liefert neun lesende Werkzeuge: Konvois und Unterkonvois auflisten, Konvoi-Details samt aller sieben Abschnitte des Marschbefehls, Fahrzeugbestand und Einzelfahrzeug, Wegpunkte in Marschreihenfolge, die gespeicherte Route, die zuletzt gemeldeten Positionen und den Marschstatus je Fahrzeug. **Gelöscht werden kann über die Schnittstelle nichts** — es existiert kein einziges Werkzeug dafür, und ein Test vergleicht die registrierte Werkzeugliste gegen eine Positivliste, damit das so bleibt. Der Server läuft im bestehenden Backend-Container; ein eigenes Deployment-Artefakt kommt nicht dazu.

  **Standardmäßig aus** (`MCP_ENABLED=false`). Eine Bestandsinstallation, die nichts konfiguriert, bekommt keinen einzigen neuen Endpunkt — weder `/mcp` noch die Well-Known-Dokumente. Das Einschalten öffnet Einsatzdaten für einen externen Modellanbieter, und diese Entscheidung gehört dem Betreiber, nicht dem Auslieferungszustand.

  **Zugriff bekommt niemand allein durch das Einschalten.** ConvoyPlan ist hier sein eigener OAuth-2.1-Authorization-Server — eine On-Prem-Installation hat keinen externen Identitätsanbieter, und Benutzer, MFA, Organisationen und Rollen liegen ohnehin in der eigenen Datenbank. Ein Client registriert sich (Dynamic Client Registration, abschaltbar über `MCP_ALLOW_DCR`), wird aber auf den Consent-Screen unter `/oauth/consent` geschickt: dort muss sich ein Benutzer anmelden (inklusive MFA), eine Organisation wählen und ausdrücklich zustimmen. Erst daraus entsteht ein Autorisierungscode. Weil sich bei DCR jeder registrieren darf, ist der vom Client angegebene Name auf dem Screen als **ungeprüft** gekennzeichnet und die tatsächlich überprüfte Zieladresse daneben als **geprüft** — sonst wäre der Bildschirm eine Einladung für ein Programm, das sich „ConvoyPlan Desktop" nennt.

  **Scopes sind eine Projektion der bestehenden Rollenhierarchie, keine zweite Berechtigungslogik.** `convoy:read` ab *beobachter*, `fleet:status` ab *fahrer*, `convoy:write` ab *planer*; ein Token kann nie mehr, als die Mitgliedschaft hergibt. Die Prüfung läuft bei jedem Aufruf frisch gegen die Datenbank: eine entzogene Mitgliedschaft oder eine herabgestufte Rolle wirkt sofort, nicht erst beim Ablauf des Tokens. Ein Token gilt für genau **eine** Organisation — auch wenn der Benutzer in mehreren Mitglied ist.

  **MCP-Tokens und ConvoyPlan-Tokens sind strukturell getrennt.** Erstere tragen `typ="mcp"` und sind an der REST-API wertlos, weil `deps._decode_token` nur `access` und `stream` akzeptiert; umgekehrt weist der MCP-Verifier alles zurück, was nicht `typ="mcp"` ist. Beide Richtungen hängen an einer Prüfung, nicht an einer Konvention, und beide haben einen Test. Jedes Token ist zusätzlich per `aud` an die eigene Instanz gebunden (RFC 8707) — ein Token von Instanz A ist an Instanz B wertlos.

  **Refresh-Tokens rotieren und erkennen Diebstahl.** Jeder Einlöseversuch gibt ein neues aus; taucht ein bereits rotiertes Token erneut auf, stirbt die ganze Familie. Dasselbe gilt für einen zweimal eingelösten Autorisierungscode. Access-Tokens sind zustandslose JWTs und laufen deshalb nach 15 Minuten ab (`MCP_ACCESS_TOKEN_TTL_MINUTES`) — ein Widerruf trifft die Refresh-Familie sofort, ein bereits ausgegebenes Access-Token bleibt höchstens diese Spanne gültig. Das ist der Preis zustandsloser Tokens und hier ausdrücklich genannt statt verschwiegen. Client-Secrets liegen Fernet-verschlüsselt statt gehasht, weil sie im Klartext zurückgebraucht werden — dasselbe Muster wie bei den MFA-Geheimnissen.

  Umgesetzt nach der MCP-Revision **2026-07-28**: Protected Resource Metadata (RFC 9728) an der Wurzel, PKCE nur mit S256, Resource Indicators (RFC 8707) und der `iss`-Parameter in jeder Autorisierungsantwort (RFC 9207) — auch in der Absage, denn ein Client muss den Absender einer Fehlermeldung genauso prüfen können wie den eines Codes. Caddy leitet `/mcp` und die OAuth-Pfade ans Backend; **bestehende Installationen mit persistiertem Caddyfile rüsten das beim nächsten Backend-Start automatisch nach**, über denselben Mechanismus, der schon die Sicherheits-Header nachgetragen hat. Die Routen sind dabei immer gesetzt, auch bei abgeschaltetem MCP — ein späteres Einschalten braucht damit keine Proxy-Änderung.

- **Die Nachfrage-Mail lässt sich jetzt bearbeiten und zur Probe verschicken.** Ihr Wortlaut stand bislang fest im Code — wer ihn ändern wollte, brauchte einen Fork. Der Demo-Reiter bekommt deshalb unten einen Editor für Betreff und HTML, mit **Vorschau** (Musterexemplar im neuen Tab) und **Testmail an mich**. Der Testversand nimmt denselben Weg wie der Betrieb: dieselbe Vorlage, dasselbe Rendering, derselbe SMTP-Versand — kommt die Testmail an, kommt auch die echte an. Er geht **ausschließlich** an die Adresse des angemeldeten Superadmins; ein frei wählbarer Empfänger machte aus dem Admin-Portal einen Versender für beliebige Adressen, mit dem Absender und dem Ruf der Installation im Rücken. Ohne konfiguriertes SMTP sagt der Knopf genau das, statt undurchsichtig zu scheitern; lehnt der Mailserver ab, steht dessen eigene Begründung („535 Authentifizierung fehlgeschlagen") in der Meldung — das ist die eigentliche Fehlersuche. Welche Platzhalter eine Vorlage kennt, liefert jetzt das Backend statt einer von Hand gepflegten Tabelle in der Oberfläche; ein unbekannter Name bleibt wörtlich stehen, ein Tippfehler verschluckt den Text also nicht.


- **Der Demo-Bereich hat jetzt einen eigenen Reiter samt Übersicht.** Bislang lag alles zur Demo im Reiter **System**, verstreut zwischen Lizenz, Update-Kanal, GitHub-Token und MFA — und die Kennzahlen gab es gar nicht: Wer wissen wollte, wie viele Interessenten in der letzten Woche dazugekommen sind oder ob die Nachfrage-Mails tatsächlich rausgehen, musste die Listen selbst abzählen. Der neue Reiter **Demo** fasst alles zusammen und sortiert es nach dem, wonach man üblicherweise sucht: Übersicht, Demo-Modus, offene Sitzungen, Interessenten, Zugangssteuerung (IP-Sperren und dauerhafte Ausnahmen). Obenauf acht Kacheln — offene Sitzungen samt derer, die in 24 Stunden ablaufen; in Demos angelegte Marschverbände als Anhaltspunkt, ob wirklich gearbeitet oder nur geschaut wurde; Interessenten gesamt mit den letzten 7 und 30 Tagen; versandte Nachfragen samt der noch folgenden; ausstehende und aufgegebene Nachfragen; Abbestellungen; gesperrte Adressen. Gezählt wird in SQL statt aus den darunter stehenden Listen: Die Interessentenliste zeigt die letzten 100 Einträge, die Übersicht soll aber auch bei mehr die richtige Zahl nennen. Die Kacheln „ausstehend" und „aufgegeben" heben sich nur dann hervor, wenn sie über 0 stehen — im Normalbetrieb sind sie leer, und eine Hervorhebung, die immer leuchtet, nimmt nach einer Woche niemand mehr wahr. Steht „ausstehend" still, nennt die Kachel auch gleich den Grund (Nachfrage abgeschaltet, kein SMTP hinterlegt).


- **Die Demo weiß jetzt, wer sie startet — und fragt nach, wenn sie vorbei ist.** Bislang war jede Demo-Sitzung anonym: eine Organisation namens „Demo A3F9C2", dazu eine grobe Herkunft aus der IP. Wer da gerade testete, ließ sich nicht sagen, und nach Ablauf war der Kontakt weg. Der Demo-Knopf führt deshalb auf ein kurzes Formular unter `/demo`: **E-Mail-Adresse ist Pflicht, Vor- und Nachname sind freiwillig** — ohne Adresse lässt sich weder zuordnen noch nachfragen, alles darüber hinaus kostet nur Abbrüche auf dem Weg in die Demo. Die Angabe landet in der neuen Tabelle `demo_leads` und **nicht** als Kennung des Demo-Benutzers; dort steht weiterhin die interne `demo-<hex>@demo.local`, sonst kollidierte der Demo-Zugang mit einem bestehenden Konto derselben Adresse. Die Kontaktzeile liegt außerhalb der Demo-Organisation, weil sie diese überleben muss: Ist die Sitzung abgelaufen, verschickt der Retention-Durchgang **einmalig** eine Nachfrage — hat alles so funktioniert wie erwartet, ist eine Frage offen geblieben, soll man sich das gemeinsam in einer Session ansehen — zu einem Zeitpunkt, an dem die Umgebung längst gelöscht ist. Ausgelöst wird sie über den in der Kontaktzeile gespiegelten Ablaufzeitpunkt, nicht über die Organisation; „+24 h" und „✕ Beenden" im Admin-Portal ziehen ihn mit. Ohne konfiguriertes SMTP unterbleibt der Versand, ohne einen Fehlversuch zu verbrauchen; ein unzustellbares Postfach hält den Durchgang nicht auf und ruht nach drei Versuchen. Statt eines Opt-in-Häkchens beim Start weist das Formular auf die einmalige Nachfrage hin, und jede Mail trägt im Fußbereich einen Abmeldelink (`/demo/abmelden`) — der Widerspruch gilt der Adresse, nicht der einzelnen Sitzung, und ein späterer Demo-Start hebt ihn nicht auf. Im Admin-Portal zeigt die Sitzungsliste jetzt den Kontakt, ein neuer Abschnitt **Interessenten aus der Demo** die Kontakte samt Stand der Nachfrage, dazu ein Schalter für die Nachfrage und ein Löschknopf je Kontakt (Löschersuchen nach DSGVO Art. 17). Aufbewahrung: `RETENTION_DEMO_LEADS_DAYS`, Standard 180 Tage.

- **Der Kartenwechsel kann GraphHopper für den Import anhalten und sich auf einen Zeitpunkt legen lassen.** Ein Wechsel auf DACH+Italien+Balkan starb nach zwölf Minuten Import am OutOfMemoryError — und das Panel hatte vorher grünes Licht gegeben: Es schrieb den Heap des laufenden GraphHopper immer gut, mit der Begründung, der Updater verkleinere ihn während des Imports. Das tat er nie, denn GraphHopper wird erst *nach* dem Import gestoppt, und eine JVM gibt reservierten Heap ohnehin nicht zurück. Panel und Updater nannten damit verschiedene Zahlen. Der Bonus zählt jetzt nur noch im neuen **Wartungsmodus**, wo er wahr wird: Dort hält der Updater GraphHopper an, *bevor* er den Graph baut — das Routing pausiert für die Dauer des Imports, dafür werden große kombinierte Karten auf knappem Speicher überhaupt erst baubar. Der freigegebene Speicher wird dabei gemessen statt gerechnet, sonst liefe der Import in den OOM-Killer statt in einen sauberen Java-Fehler. Weil das Routing dabei ausfällt, lässt sich ein Wechsel außerdem auf einen **Zeitpunkt** legen (höchstens 30 Tage voraus, ein Abbruch sticht den Termin). Und die Speicherprüfung sitzt jetzt wie die Plattenplatzprüfung vor dem Download statt zwölf Minuten später.

### Changed

- **Die Endpunkte der E-Mail-Vorlagen tragen eine Kennung im Pfad.** Bis auf die Zugangsdaten-Mail gab es nichts zu verwalten, entsprechend hieß es schlicht `/api/admin/email-template`. Mit der Nachfrage-Mail sind es zwei und weitere werden folgen — aus `…/email-template` wird deshalb `…/email-templates/{kind}` (`password`, `demo_followup`), dazu ein Verzeichnis unter `…/email-templates`. Ein zweites Modul mit denselben vier Endpunkten wäre die Alternative gewesen. Betrifft nur das Admin-Portal, das im selben Image ausgeliefert wird; die hinterlegten Vorlagen bestehender Installationen bleiben unangetastet — die Zugangsdaten-Mail behält bewusst ihre historischen Schlüsselnamen in `system_settings`, ein Umbenennen hätte angepasste Vorlagen stillschweigend auf den Standard zurückgesetzt.
...
- **Eine abgelehnte Dokumentenanfrage über die KI-Schnittstelle nennt wieder den Grund.** Die Absage („kein Konvoi mit dieser ID in dieser Organisation", „ohne gültige Lizenz nur lesend") wurde vom MCP-SDK verschluckt und durch ein nacktes „Error creating resource from template …" ersetzt, während im Log ein Absturz mit Traceback stand — für eine ganz normale Zugriffsprüfung. Betroffen waren die drei Dokumente (Marschbefehl-PDF, Route-GPX, Konvoi-JSON); an den Werkzeugen war die Meldung immer korrekt. Der Zugriffsschutz selbst war nie betroffen: abgelehnt wurde in jedem Fall.

- **Der Reiterbalken im Admin-Portal lief auf schmalen Bildschirmen über.** Mit dem achten Reiter passen die Beschriftungen auf einem Telefon nicht mehr nebeneinander; sie wurden gestaucht, bis sie unleserlich waren. Der Balken scrollt jetzt seitlich, wie es breite Tabellen in den Karten ohnehin tun — ein Umbruch in eine zweite Zeile hätte die Unterkante zerrissen, an der die aktive Lasche hängt.
...
- **Schwarze Keile quer über die Karte, nachdem die Maske der Region folgte.** Der Umriss stimmte nach dem vorigen Fix, aber über Adria und Balkan lagen große schwarze Dreiecke. Ursache war eine falsche Annahme in genau diesem Fix: Die äußeren Ringe der Bestandteile wurden nur aneinandergehängt, weil überlappende Löcher angeblich dieselbe Aussparung ergeben wie ein vereinigtes Polygon. Das stimmt nicht — MapLibre trianguliert Flächen mit earcut, und earcut setzt voraus, dass die Löcher eines Polygons einander nicht schneiden. Mit earcut selbst nachgemessen (`deviation()`, 0 = korrekt): disjunkte Löcher `0.000e+0`, überlappende `5.381e-2`. Bei hunderten Ringen echter Landesumrisse zerfällt die Triangulierung entsprechend.
  ...
  Überlappung ist dabei kein Sonderfall: Geofabrik schneidet seine Extracts mit einem Puffer über die Verwaltungsgrenze hinaus, damit Wege an der Grenze nicht zerrissen werden — benachbarte Regionen greifen deshalb immer ineinander, und `europe/dach` deckt sich großflächig mit `europe/germany`, `europe/austria` und `europe/switzerland`. Der Umriss wird jetzt serverseitig zu **einer überschneidungsfreien Fläche vereinigt** (shapely, im Backend ohnehin vorhanden); ungültige Eingaben werden vorher repariert, damit eine Selbstberührung im vereinfachten Umriss nicht die ganze Maske kostet.
