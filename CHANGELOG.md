...
- **Karten zeigen den Deutschland-Fokus und folgen dem Hell-/Dunkel-Design.** Die Routenberechnung ist auf Deutschland begrenzt (der GraphHopper-Graph enthält nur das deutsche Straßennetz), die Karte sah aber überall gleich aus — ein Ziel jenseits der Grenze war erst an der fehlgeschlagenen Berechnung zu erkennen. Alles außerhalb der Bundesgrenze wird deshalb jetzt abgedunkelt bzw. ausgegraut und die Grenze selbst als feine Linie gezogen; das Ausland bleibt als Orientierung sichtbar, tritt aber deutlich zurück. Gleichzeitig richtet sich die Karte nach dem gewählten Design: im Dunkelmodus werden die Kacheln getönt, damit die Karte nicht mehr als heller Block in der dunklen Oberfläche steht — Route, Sperrungen, Verkehrslage und Fahrzeug-Marker behalten ihre volle Leuchtkraft. Umgeschaltet wird ohne Neuladen der Karte. Gilt für die Planungs- und Tracking-Karte ebenso wie für die Leitstellen-Übersicht und die Gebietsauswahl im Admin-Portal. Die Bundesgrenze (`static/geo/deutschland.geojson`, ~52 kB) ist aus den bereits vorhandenen Kreisgrenzen abgeleitet — dieselbe Quelle und Namensnennung (BKG, dl-de/by-2-0), kein zusätzlicher Kartendienst. Lässt sie sich nicht laden, bleibt die Karte ohne Maske voll bedienbar.
