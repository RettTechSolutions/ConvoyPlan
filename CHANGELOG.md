...
- **Ein Umsortieren der Wegpunkte änderte die Route nicht und war nach der Berechnung wieder weg.** Die Reihenfolge ließ sich ziehen und wurde auch gespeichert — die Routenberechnung ignorierte sie und schrieb sie anschließend zurück auf den alten Stand.
  ...
  Welcher Wegpunkt eingeordnet wird, entscheidet dabei seine **Herkunft** und nicht seine Position: Ein Vorschlag der Anwendung wird beim Anlegen als „noch nicht einsortiert" vermerkt, und der Vermerk verfällt, sobald er einen Platz hat — durch die Berechnung, durch Ziehen in der Liste oder durch ein ausdrücklich gesetztes Ziel. Ein Technischer Halt, den jemand bewusst als **letzten** Wegpunkt setzt, bleibt damit auch dort. Bestehende Konvois behalten bei der ersten Berechnung nach dem Update genau das bisherige Verhalten (Migration `0045`).
