...
|---|---|---|---|---|
| T1 ✅ | **Audit-Log** — *umgesetzt.* | A.8.15 Logging, A.5.28 Beweissicherung, 27701 Betroffenenrechte | Append-only `audit_logs`-Tabelle (Migration 0018) + Service `audit.record()`. Erfasst Login (Erfolg/Fehlschlag), MFA-Änderung, Passwortänderung/-Reset, Benutzer-/Org-CRUD, Lizenzaktivierung **sowie Konvoi-/Marschbefehl-Änderungen (Anlegen/Ändern/Löschen inkl. Teilverbände) und Share-Link-Erstellung/-Widerruf** inkl. Akteur, Ziel, IP, User-Agent. Lesbar über `GET /api/admin/audit-log`. | Mittel |
...
| T10 ✅ | **SCA / Schwachstellen­überwachung** — *umgesetzt.* | A.8.8 Technisches Schwachstellen­management | `.github/dependabot.yml` (pip/npm/actions/docker) + **blockierender** CI-Job `dependency-audit` (`pip-audit --strict` + `npm audit --audit-level=high`) sowie **Container-Image-Scan** `container-scan` (Trivy über alle drei Images: Backend, Frontend, Updater; `--severity HIGH,CRITICAL --ignore-unfixed --exit-code 1`). **Patch-SLA** je Schweregrad in `SECURITY.md` dokumentiert. | Quick Win |
...
16. ✅ **T10 (Rest)** `dependency-audit` blockierend + Trivy-Image-Scan + Patch-SLA — *umgesetzt.*
