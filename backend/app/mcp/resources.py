...
async def _export(konvoi_id: str, fn) -> tuple[bytes, str]:
...
def register(mcp) -> None:
    ...
    async def marschbefehl_pdf(konvoi_id: str) -> str:
        content, _name = await _export(konvoi_id, routing_routes.export_pdf)
    ...
    async def route_gpx(konvoi_id: str) -> str:
        content, _name = await _export(konvoi_id, routing_routes.export_gpx)
    ...
    async def konvoi_json(konvoi_id: str) -> str:
        content, _name = await _export(konvoi_id, routing_routes.export_json)
        return content.decode()
